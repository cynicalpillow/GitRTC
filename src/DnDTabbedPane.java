...
public class DnDTabbedPane extends JTabbedPane {
    ...
    private void convertTab(TabTransferData a_data, int a_targetIndex) {
        ...
        //MODIFICATION: Remove from tabs list, and switch the location
        if(a_targetIndex == getTabCount()){
            int length = e.tabs.size()-1;
            ButtonTabComponent b = e.tabs.get(sourceIndex);
            e.tabs.remove(sourceIndex);
            e.tabs.add(length, b);
        } else if(sourceIndex > a_targetIndex) {
            int index = Math.min(sourceIndex, e.tabs.size() - 1);
            index = Math.max(index, 0);
            int targetIdx = Math.min(a_targetIndex, e.tabs.size() - 1);
            targetIdx = Math.max(targetIdx, 0);
            ButtonTabComponent b = e.tabs.get(index);
            e.tabs.remove(index);
            e.tabs.add(targetIdx, b);
        } else {
            int index = Math.min(sourceIndex, e.tabs.size() - 1);
            index = Math.max(index, 0);
            int targetIdx = Math.min(a_targetIndex-1, e.tabs.size() - 1);
            targetIdx = Math.max(targetIdx, 0);
            ButtonTabComponent b = e.tabs.get(index);
            e.tabs.remove(index);
            e.tabs.add(targetIdx, b);
